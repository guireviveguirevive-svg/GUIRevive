...
- repair: Fix failed test cases (parse failed cases from All_cases.csv)
...
class DroidBotRepairRunner:
    ...
    def __init__(self, args):
        self.cases_csv = args.cases_csv  # Path to All_cases.csv
        self.data_dir = '.'  # Data directory (current directory)
        ...
        # select_apks directory (hardcoded)
        self.select_apks_dir = 'droidbot/select_apks'

        # Filter by app package name (optional)
    ...
    def _default_log_dir(self) -> str:
        return os.path.join(self.data_dir, 'logs_repair')
    ...
    def parse_cases_csv(self, csv_path: str) -> List[Dict]:
        """
        Parse All_cases.csv, extract test cases
        ...
        with open(csv_path, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                ...
                mismatch = row.get('Mismatch', '').strip()

                if app and record_app and replay_app and run_count:
                    cases.append({
                        'app': app,
        ...
        logger.info(f"Parsed {len(cases)} test cases from CSV")
        return cases
    ...
    def suffix_to_apk_path(self, suffix: str, apk_dir: str) -> Optional[str]:
        """
        Convert app version name from CSV to APK path

        The suffix in the CSV is already sanitized (extracted from directory name)
        So we just need to apply the same sanitize_suffix processing to the APK filename and compare directly

        For example:
        - suffix (from CSV): "Notes__Privacy_Friendly__v1_3_0"
        ...
        Args:
            ...
            apk_dir: Directory to search for APKs

        Returns:
            APK file path, or None if not found
        """
        # Get all APK files
        search_pattern = os.path.join(apk_dir, "**/*.apk")
    ...
    def generate_tasks(self) -> List[Dict]:
        """
        Generate repair tasks from All_cases.csv

        Directory mapping:
        - record_output: {data_dir}/{app}/record_output_{record_app}_run{N}
        - failed_replay_output: {data_dir}/{app}/replay_output_{replay_app}_run{N}_for_{record_app}
        - repair_output: {data_dir}/{app}/repair_output_{replay_app}_run{N}_for_{record_app}
        """
        cases = self.parse_cases_csv(self.cases_csv)
        if not cases:
            return []
        ...
        for case in cases:
            ...
            # Filter by apk_base if specified
            if self.apk_base and app != self.apk_base:
                continue

            # Build directory names (relative to data_dir/app)
            app_dir = os.path.join(self.data_dir, app)
            record_output_dir = os.path.join(app_dir, f"record_output_{record_app}_run{run_count}")
            failed_replay_output_dir = os.path.join(app_dir, f"replay_output_{replay_app}_run{run_count}_for_{record_app}")
            repair_output_dir = os.path.join(app_dir, f"repair_output_{replay_app}_run{run_count}_for_{record_app}{self.repair_output_dir_suffix}")
            ...
            # APK is in select_apks/{app}/{version}.apk
            app_apk_dir = os.path.join(self.select_apks_dir, app)
            apk_path = self.suffix_to_apk_path(replay_app, app_apk_dir)
            if not apk_path:
                logger.info(f"Skip: APK not found: {replay_app} (in {app_apk_dir})")
                continue
    ...
    def run(self):
        ...
        logger.info(f"Cases CSV: {self.cases_csv}")
        ...
        if not os.path.exists(self.cases_csv):
            logger.error(f"Cases CSV does not exist: {self.cases_csv}")
            return
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='DroidBot Repair Batch Runner - Parse cases from All_cases.csv and repair them'
    )

    parser.add_argument('--cases-csv', default='All_cases.csv',
                        help='Path to All_cases.csv containing test cases')
    parser.add_argument('--apk-base', default=None,
                        help='Filter cases by app package name (optional, process all if not specified)')
    ...
    parser.add_argument('--log-dir', default=None,
                        help='Log directory (default at data-dir/logs_repair)')
