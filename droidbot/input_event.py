...
class SelectEvent(UIEvent):
    ...
    def send(self, device):
        if self.u2 is not None:
            ...
            # Fall back to the original coordinate method
            x, y = UIEvent.get_xy(x=self.x, y=self.y, view=self.view)
...
class ScrollEvent(UIEvent):
    ...
    def send(self, device, probe=False):
        ...
        Args:
            ...
            probe: Whether in probe mode. When True, uses smaller swipe distance (1/20=5%); when False, uses normal distance (2/5=40%)
        """
        # Probe mode uses small swipe distance (5%), normal mode uses large distance (40%)
        # 5% swipe is usually enough to make some elements enter/leave the viewport, triggering state_str changes
        scroll_ratio = 1 / 5 if probe else 2 / 5
        ...
        if self.u2 is not None:
            ...
            start_x, start_y = x, y
            end_x, end_y = x, y
            duration = 150 if probe else 500  # Probe mode uses shorter duration
